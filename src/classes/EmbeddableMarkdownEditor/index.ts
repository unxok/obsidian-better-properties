 ...
 * - Added fix for Workspace.activeEditor not properly updating when moving focus from the EMDE to the note editor in the same leaf.
...
function resolveEditorPrototype(app: App) {
	const fakeFileName = "_______BETTER_PROPERTIES_FAKE_FILE_NAME__________";
	const fakeFile = {
		...
		name: fakeFileName + ".md",
		parent: null,
		path: fakeFileName + ".md",
		saving: false,
		setPath: () => {},
		stat: {
			ctime: -1,
...
export type EmbeddableMarkdownEditor = ReturnType<
	typeof createEmbeddableMarkdownEditor
>;
...
export const createEmbeddableMarkdownEditor = ({
	app,
	container,
	options,
	filePath,
	...
	return new EmbeddableMarkdownEditor(app, container, options, filePath);
};
