...
export class BpJsApi {
	...
	async require(path: string): Promise<unknown> {
		const { vault } = this.plugin.app;
		const file = vault.getFileByPath(path);
		if (!file) {
			throw new Error(
				`Failed to require module: File not found at path "${path}"`
			);
		}
		const module: {
			default?: unknown;
		} = {};
		eval(await vault.cachedRead(file)); // should set module.default
